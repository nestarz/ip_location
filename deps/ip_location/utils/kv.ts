...
export const createKv = (buildId?: string) => {
  ...
  const namespace = ["_frsh", "bin", buildId ?? BUILD_ID];
  ...
  return {
    getFile: async (file: string) => {
      ...
      console.log("get", filepath, metadata);
    ...
    list: async () => {
      const kv = await kvPromise;
      const items = [];
      for await (const item of kv.list({ prefix: namespace }))
        items.push(item.key);
      console.log(items);
      return items;
    },
    deleteItem: async (key: string) => {
      ...
      for await (const item of kv.list({ prefix: [...namespace, key] })) {
